...
if (plugin.canChangeOptions()) {
	...
	theWebUI.telegramWasChanged = function() {
		...
		for (const event of ['added', 'resumed', 'finished', 'removed']) {
...
plugin.onLangLoaded = function() {
	...
	const events = ['added', 'resumed', 'finished', 'removed'];
		...
		$('<fieldset>').append(
			...
			$('<div>').addClass('row').append(
				$('<div>').addClass('col-12').append(
					$('<button>').attr({type: 'button'}).addClass('OK').css({width: 'max-content', minWidth: '180px', whiteSpace: 'nowrap', color: 'var(--menu-connect-line-color, var(--text-color, inherit))', opacity: 1, filter: 'none', cursor: 'pointer', fontWeight: 'bold'}).on('click', () => { plugin.testTelegram(); return false; }).text(theUILang.telegramTest),
